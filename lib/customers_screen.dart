...
class _CustomerFormDialogState extends ConsumerState<_CustomerFormDialog> {
  ...
  void initState() {
    ...
    _dueDateCtrl =
        TextEditingController(text: c?.dueDatePeriod?.toString() ?? '');
    _streetNameCtrl = TextEditingController(text: c?.streetName ?? '');
    _additionalStreetCtrl =
        TextEditingController(text: c?.additionalStreetName ?? '');
    ...
    _citySubdivisionCtrl =
        TextEditingController(text: c?.citySubdivisionName ?? '');

    _isCustomer = c?.isCustomer ?? true;
    _isSupplier = c?.isSupplier ?? false;
    _isTaxExempt = c?.isTaxExempt ?? false;
    _selectedCountryId =
        (c?.countryId != null && c!.countryId! > 0) ? c.countryId : null;
